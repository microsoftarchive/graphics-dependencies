...
//
HRESULT WASAPIRenderer::InitializeAudioDeviceAsync(SampleReceivedUserCallback sampleReceivedUserCallback, void *userData)
{
...
{
    if (m_DeviceStateChanged->GetState() != DeviceStateActivated)
    {
...
HRESULT WASAPIRenderer::OnStartPlayback( IMFAsyncResult* pResult )
{
    ...
    if (SUCCEEDED( hr ))
    {
        m_DeviceStateChanged->SetState( DeviceStatePlaying, S_OK, true );
...
{
    if ( (m_DeviceStateChanged->GetState() != DeviceStatePlaying) &&
         (m_DeviceStateChanged->GetState() != DeviceStatePaused) &&
...
HRESULT WASAPIRenderer::PausePlaybackAsync()
{
    ...
    m_DeviceStateChanged->SetState( DeviceStatePausing, S_OK, false );
...
{
    m_AudioClient->Stop();
    m_DeviceStateChanged->SetState( DeviceStatePaused, S_OK, true );
    return S_OK;
}
...
exit:
    ...
    {
        m_DeviceStateChanged->SetState( DeviceStateUnInitialized, hr, false );
        SAFE_RELEASE( m_AudioClient );
        SAFE_RELEASE( m_AudioRenderClient );
        SAFE_RELEASE( m_SampleReadyAsyncResult );

        hr = InitializeAudioDeviceAsync(m_sampleReceivedUserCallback, m_userData);
    }
...
HRESULT WASAPIRenderer::GetToneSample( UINT32 FramesAvailable )
{
    ...
    else if (m_ToneSource->GetBufferLength() <= ( FramesAvailable * m_MixFormat->nBlockAlign ))
    {
        ...
        if (SUCCEEDED( hr ))
        {
			m_sampleReceivedUserCallback(m_userData, Data, FramesAvailable, ActualBytesToRead, false);
