...
std::string PathImpl::currentImpl()
{
	return ".\\";
	//std::string result;
	//DWORD len = 0;//GetCurrentDirectoryW(0, NULL);
	//if (len > 0)
	//{
	//	Buffer<wchar_t> buffer(len);
	//	DWORD n = 0;//GetCurrentDirectoryW(len, buffer.begin());
	//	if (n > 0 && n <= len)
	//	{
...
{
	Buffer<wchar_t> buffer(MAX_PATH_LEN);
	DWORD n = 0;//GetTempPathW(static_cast<DWORD>(buffer.size()), buffer.begin());
	if (n > 0)
	{
		n = 0;//GetLongPathNameW(buffer.begin(), buffer.begin(), static_cast<DWORD>(buffer.size()));
...
std::string PathImpl::expandImpl(const std::string& path)
{
	...
	Buffer<wchar_t> buffer(MAX_PATH_LEN);
	DWORD n = 0;//ExpandEnvironmentStringsW(upath.c_str(), buffer.begin(), static_cast<DWORD>(buffer.size()));
...
void PathImpl::listRootsImpl(std::vector<std::string>& roots)
{
	roots.clear();
	wchar_t buffer[128];
	DWORD n = 0;//GetLogicalDriveStringsW(sizeof(buffer)/sizeof(wchar_t) - 1, buffer);
	wchar_t* it  = buffer;
