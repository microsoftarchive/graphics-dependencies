	...
	_address(0)
{
	...
	_memHandle = CreateFileMappingFromApp(INVALID_HANDLE_VALUE, NULL, _mode, _size, utf16name.c_str());
#else
	...
	_address(0)
{
	...
	_fileHandle = CreateFile2(utf16name.c_str(), fileMode, shareMode, OPEN_EXISTING, NULL);
#else
	...
	_memHandle = CreateFileMappingFromApp(_fileHandle, NULL, _mode, 0, NULL);
	if (!_memHandle)
	{
...
void SharedMemoryImpl::map()
{
	...
	LPVOID addr = MapViewOfFileFromApp(_memHandle, access, 0, _size);
	if (!addr)
