...
{
	_mutex = CreateMutexEx(NULL, NULL, 0, SYNCHRONIZE | MUTEX_ALL_ACCESS);
	if (_mutex == NULL)
		throw SystemException("cannot create reader/writer lock");

	_readEvent = CreateEventEx(NULL, NULL, CREATE_EVENT_MANUAL_RESET, EVENT_ALL_ACCESS | SYNCHRONIZE);
	if (_readEvent == NULL)
		throw SystemException("cannot create reader/writer lock");

	_writeEvent = CreateEventEx(NULL, NULL, CREATE_EVENT_MANUAL_RESET, EVENT_ALL_ACCESS | SYNCHRONIZE);
	if (_writeEvent == NULL)
		throw SystemException("cannot create reader/writer lock");
}
