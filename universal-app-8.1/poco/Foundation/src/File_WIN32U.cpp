...
public:
	FileHandle(const std::string& path, const std::wstring& upath, DWORD access, DWORD share, DWORD disp)
	{
		...
		_h = CreateFile2(upath.c_str(), access, share, disp, NULL);
...
bool FileImpl::canReadImpl() const
{
	...
	DWORD attr = GetFileAttributesEx(_upath.c_str(), GetFileExInfoStandard, &fileAtt);//GetFileAttributesW(_upath.c_str());
	if (attr == 0xFFFFFFFF)
	{
		switch (GetLastError())
		{
...
void FileImpl::setSizeImpl(FileSizeImpl size)
{
	...
	LARGE_INTEGER li;
	li.QuadPart = size;
	if (SetFilePointerEx(fh.get(), li, NULL, FILE_BEGIN) == -1)
		handleLastErrorImpl(_path);
...
void FileImpl::copyToImpl(const std::string& path) const
{
	...
	if(CopyFile2(_upath.c_str(), upath.c_str(), NULL) != 0)
	{
		FileImpl copy(path);
...
void FileImpl::renameToImpl(const std::string& path)
{
	...
	if(MoveFileEx(_upath.c_str(), upath.c_str(), 0) == 0)
		handleLastErrorImpl(_path);
}
...
bool FileImpl::createFileImpl()
{
	...
	HANDLE hFile = CreateFile2(_upath.c_str(), GENERIC_WRITE, 0, 0, NULL);
