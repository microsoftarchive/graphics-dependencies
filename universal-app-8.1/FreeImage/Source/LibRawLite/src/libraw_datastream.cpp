    ...
    , pView_(NULL)
{
	...
	HANDLE hFile = CreateFile2(sFile, GENERIC_READ, 0, OPEN_EXISTING, NULL);
...
void LibRaw_windows_datastream::Open(HANDLE hFile)
{
	...
	hMap_ = ::CreateFileMappingFromApp(hFile, 0, PAGE_READONLY, 0, 0);
	...
	pView_ = ::MapViewOfFileFromApp(hMap_, FILE_MAP_READ, 0, (size_t)cbView_);
    if (pView_ == NULL)	
