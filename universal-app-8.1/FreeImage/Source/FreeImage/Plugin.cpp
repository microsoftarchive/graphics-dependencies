		...
		if (s_plugins) {
			...
			*/
			...
			if (!load_local_plugins_only) {
              ...
              mbtowc(wbuffer, buffer, strlen(buffer));
							HINSTANCE instance = LoadPackagedLibrary(wbuffer, 0);
...
FreeImage_RegisterExternalPlugin(const char *path, const char *format, const char *description, const char *extension, const char *regexpr) {
	if (path != NULL) {
    wchar_t wpath[460];
    mbtowc(wpath, path, strlen(path));
		HINSTANCE instance = LoadPackagedLibrary(wpath, 0);
